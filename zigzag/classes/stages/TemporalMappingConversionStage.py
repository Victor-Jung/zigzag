...
class TemporalMappingConversionStage(Stage):
    ...
    def check_layer(layer):
        ...
        if not layer.core_allocation:
            ...
            raise ValueError()
        if not layer.user_temporal_mapping:
            logger.critical(f"Layer {layer} has no user-defined temporal mapping.")
            raise ValueError(f"Layer {layer} has no user-defined temporal mapping.")
    ...
    def run(self):
        ...
        """
        temporal_mapping = TemporalMapping(self.layer.user_temporal_mapping, self.layer)
        logger.info(f"Using user-defined temporal mapping:\n\tI :{temporal_mapping.mapping_dic_origin['I']}\n\tW :{temporal_mapping.mapping_dic_origin['W']}\n\tO :{temporal_mapping.mapping_dic_origin['O']}")
